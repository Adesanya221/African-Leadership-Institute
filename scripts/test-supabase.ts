 ...
 *
 * Or compile & run:
 *   npx tsc scripts/test-supabase.ts --esModuleInterop && node scripts/test-supabase.js
 */
...
async function testPaymentSuccessUpdate(testRef: string) {
...
async function testReadBack(testRef: string) {
...
async function cleanup(testRef: string) {
...
async function testTableSchema() {
  log('TEST 0: Table Schema Check', COLORS.bold + COLORS.cyan);
  ...
  log('✅ PASS: Registrations table is accessible', COLORS.green);

  // Try to infer columns from a real row or just describe
  const { data: columns, error: colErr } = await supabase.rpc('');
  // There's no direct column listing in supabase-js, but we can try insert+rollback
